...
export default function App() {
  ...
  const fetchCharacter = async (idStr: string) => {
    ...
    setLoadingDDB(true);
    try {
      const url = `https://character-service.dndbeyond.com/character/v5/character/${encodeURIComponent(
        id
      ...
      const dexStat = Array.isArray(ddb?.stats)
        ...
        : null;
      const dexScore: number | null =
        typeof dexStat?.value === 'number'
          ? dexStat.value
          : typeof dexStat?.score === 'number'
          ? dexStat.score
          : null;
      ...
      <div className='max-w-5xl mx-auto grid gap-6'>
        ...
        <section className='grid md:grid-cols-2 gap-4'>
          <div className='bg-white rounded-2xl shadow p-4 md:p-5'>
            <h2 className='font-semibold text-lg mb-3'>
              Add Character (D&D Beyond)
            </h2>
            <div className='flex gap-2'>
              <input
                className='flex-1 rounded-xl border border-slate-300 px-3 py-2'
                placeholder='Enter D&D Beyond Character ID'
                value={ddbId}
                onChange={(e) => setDdbId(e.target.value)}
                onKeyDown={(e) => {
                  if (e.key === 'Enter') fetchCharacter(ddbId);
                }}
              />
              <button
                className='rounded-xl px-4 py-2 bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-50'
                onClick={() => fetchCharacter(ddbId)}
                disabled={loadingDDB}
              >
                {loadingDDB ? 'Adding…' : 'Add'}
              </button>
            </div>
          ...
          <div className='bg-white rounded-2xl shadow p-4 md:p-5'>
            ...
            <div className='grid grid-cols-2 gap-2 md:grid-cols-4'>
              ...
              <input
                ...
                type='number'
                value={mHP}
                onChange={(e) => setMHP(parseInt(e.target.value || '0'))}
              />
              <input
                className='rounded-xl border border-slate-300 px-3 py-2'
                placeholder='AC'
                ...
                value={mAC}
                onChange={(e) =>
                  setMAC(e.target.value === '' ? '' : parseInt(e.target.value))
                }
              />
              <input
                className='rounded-xl border border-slate-300 px-3 py-2'
                ...
                type='number'
                value={mDexMod}
                onChange={(e) => setMDexMod(parseInt(e.target.value || '0'))}
              />
              <button
            ...
            <p className='text-xs text-slate-500 mt-2'>
              Dex Mod affects initiative rolls (d20 + Dex Mod).
            </p>
          </div>
        </section>
        ...
        <section className='bg-white rounded-2xl shadow p-4 md:p-5'>
          <div className='flex flex-wrap items-center gap-2'>
            <button
              className='rounded-xl px-3 py-2 bg-slate-800 text-white hover:bg-slate-900'
              onClick={rollAllMonsters}
            >
              Roll All Monsters
            </button>
            <button
              className='rounded-xl px-3 py-2 bg-slate-200 hover:bg-slate-300'
            ...
            <div className='ml-auto flex gap-2'>
              <button
                className='rounded-xl px-3 py-2 bg-sky-600 text-white hover:bg-sky-700'
                onClick={prevTurn}
              >
                Prev Turn
              </button>
        ...
        <section className='bg-white rounded-2xl shadow'>
          <div className='p-4 md:p-5 border-b border-slate-100 flex items-center justify-between'>
            ...
            <span className='text-xs text-slate-500'>
              Click a number to edit; use 🎲 to roll
            </span>
          </div>
          <ul className='divide-y divide-slate-100'>
              ...
              return (
                ...
                >
                  <div className='flex items-center gap-3 w-full md:w-1/3'>
                    ...
                    <div className='flex-1'>
                      ...
                      <div className='text-xs text-slate-500'>
                        {c.source === 'ddb' ? (
                          <span>
                            DEX: {c.dex ?? '—'} (mod {c.dexMod >= 0 ? '+' : ''}
                            {c.dexMod})
                          </span>
                        ) : (
                          <span>
                            AC: {(c as any).ac ?? '—'} · Dex Mod:{' '}
                            {c.dexMod >= 0 ? '+' : ''}
                            {c.dexMod}
                          </span>
                        )}
                  ...
                  <div className='flex items-center gap-2 md:ml-auto'>
                    <button
                      className='rounded-lg px-2 py-1 bg-slate-200 hover:bg-slate-300'
                    ...
                    {hasHP && (
                      <div className='flex items-center gap-2'>
                        <span className='text-sm text-slate-600'>
                          HP {(c as any).hp}/{(c as any).maxHp}
                        </span>
                        <button
                          className='rounded-lg px-2 py-1 bg-rose-600 text-white hover:bg-rose-700'
                          onClick={() => damage(c.id, 1)}
                        >
                          -1
                        </button>
                        <button
                          className='rounded-lg px-2 py-1 bg-emerald-600 text-white hover:bg-emerald-700'
                          onClick={() => heal(c.id, 1)}
                        >
                          +1
                        </button>
                    ...
                    <button
                      className='rounded-lg px-2 py-1 bg-white border border-slate-300 hover:bg-slate-50'
            ...
            {ordered.length === 0 && (
              <li className='p-6 text-slate-500 text-sm'>
        ...
        <section className='text-xs text-slate-500'>
          <ul className='list-disc ml-5 space-y-1'>
            <li>
              Characters are pulled from D&D Beyond using the public
              character-service API you provided. Some character features that
              affect initiative aren’t auto-applied—set their initiative
              manually or use 🎲 with the shown Dex Mod.
            </li>
            <li>Monster initiative uses a simple roll: d20 + Dex Mod.</li>
            <li>
              LocalStorage keeps your roster and round/turn between reloads.
            </li>
          </ul>
        </section>
      </div>
