...
// Simple, single-file React app for managing D&D combat.
// Features
// - Add Characters by D&D Beyond Character ID (fetches name & DEX)
// - Add Monsters manually (name, HP, AC, Dex Mod)
// - Roll initiative for monsters; set or roll initiative for characters
// - Sort by initiative and track turns; adjust HP; remove combatants
...
// Types
type Character = {
  id: string; // D&D Beyond character ID (string to preserve as-entered)
  name: string;
  dex: number | null;
  dexMod: number; // derived from dex or user override
  source: "ddb";
};
...
export default function App() {
  ...
  const ordered = useMemo(() => {
    ...
    // Sort: higher initiative first; tie-breaker: d20 roll order preserved
  ...
  const fetchCharacter = async (idStr: string) => {
    ...
    setLoadingDDB(true);
    try {
      const url = `https://character-service.dndbeyond.com/character/v5/character/${encodeURIComponent(id)}?includeCustomItems=true`;
      const res = await fetch(url);
      if (!res.ok) throw new Error(`Fetch failed (${res.status})`);
      const data = await res.json();
      // DDB v5 shape: data.data.id, data.data.name, data.data.stats (array with abilityId 2 = DEX)
      const ddb = data?.data;
  ...
  const sortByInitiative = () => {
    // Sorting is derived in `ordered`, but reseat turnIndex to top-most non-null
  ...
  const nextTurn = () => {
    ...
    // advance to next with initiative set; loop safety
    let guard = 0;
  ...
  const damage = (id: string, amt: number) => {
    setCombatants((prev) => prev.map((c) => {
      if (c.id !== id) return c;
      if ((c as any).hp == null) return c; // Characters don't track HP here
      ...
      <div className="max-w-5xl mx-auto grid gap-6">
          ...
          <div className="bg-white rounded-2xl shadow p-4 md:p-5">
            ...
            <div className="flex gap-2">
              <input
                ...
                onKeyDown={(e) => { if (e.key === 'Enter') fetchCharacter(ddbId); }}
              />
              <button
                ...
                <li key={c.id} className={`p-4 md:p-5 flex flex-col md:flex-row md:items-center gap-3 ${isTurn ? 'bg-indigo-50' : ''}`}>
                  ...
                  <div className="flex items-center gap-2 md:ml-auto">
                    ...
                    <input
                      ...
                      placeholder="Set"
                      value={c.initiative ?? ""}
                      onChange={(e) => setInitiative(c.id, e.target.value === "" ? null : parseInt(e.target.value))}
                    />
                    {hasHP && (
        ...
        <section className="text-xs text-slate-500">
          <ul className="list-disc ml-5 space-y-1">
            <li>Characters are pulled from D&D Beyond using the public character-service API you provided. Some character features (e.g., feats, magic items) that affect initiative aren’t auto-applied—set their initiative manually or use 🎲 with the shown Dex Mod.</li>
            <li>Monster initiative uses a simple roll: d20 + Dex Mod.</li>
            <li>Use localStorage to keep your roster and round/turn between reloads.</li>
          </ul>
        </section>
      </div>
    </div>
